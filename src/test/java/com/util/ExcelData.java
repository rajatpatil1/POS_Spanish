...
public class ExcelData {
	...
	public static void writeExcelData(ArrayList<Object[]> data) throws IOException {
		try {
			...
			r1c2.setCellValue("Transaction No./ No. de Transacción");
			XSSFCell r1c3 = row1.createCell(1);
			r1c3.setCellValue("PIN number / Clave / Folio");
			...
			String currentDirectory = System.getProperty("user.dir");
			File pathToFile = new File(currentDirectory + "/ReceiptDetails.xlsx");
	...
	public static void writeExcelConsoleLogs(ArrayList<Object[]> data) throws IOException {
		try {
			XSSFWorkbook workbook = new XSSFWorkbook();
			XSSFSheet sheet = createSheet(workbook, "Sheet 1", false);
			XSSFRow row1 = sheet.createRow(0);
			XSSFCell r1c2 = row1.createCell(0);
			r1c2.setCellValue("UserName");
			XSSFCell r1c3 = row1.createCell(1);
			r1c3.setCellValue("Console Message");
			XSSFCell r1c4 = row1.createCell(2);
			r1c4.setCellValue("Result");
			// Iterate over data and write to sheet
			ArrayList<Object[]> keyIds = data;
			// ArrayList<Object[]> sessionIds = SessionUrl;
			int rowid = 1;
			...
			// Save excel to HDD Drive
			String currentDirectory = System.getProperty("user.dir");
			File pathToFile = new File(currentDirectory + "/ConsoleLogResult.xlsx");
			if (!pathToFile.exists()) {
				pathToFile.createNewFile();
			}
			FileOutputStream fos = new FileOutputStream(pathToFile);
